 ...
 * - Subdomain Ingress (*.spicy-llama.duckdns.org / https): Standard port 443 HTTPS.
 * - Local LAN (http / hostname / LAN IP): Direct pure HTTP ports.
 */
...
function getDuckDnsRootDomain(hostname) {
  if (!hostname || !hostname.endsWith('.duckdns.org')) {
...
function adaptServiceUrl(targetHref, currentOrigin) {
  ...
  try {
    ...
    const isLocalOrigin = currentUrl.protocol === 'http:' && 
    ...
    if (currentUrl.protocol === 'https:' || currentUrl.hostname.includes('.duckdns.org') || currentUrl.hostname.includes('.ts.net')) {
      if (targetUrl.port && PORT_TO_SERVICE[targetUrl.port]) {
        const serviceName = PORT_TO_SERVICE[targetUrl.port];
        const rootDomain = getDuckDnsRootDomain(currentUrl.hostname);
