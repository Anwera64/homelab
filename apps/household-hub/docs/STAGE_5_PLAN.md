...
**Slice 2 — built (16 September 2026).** Backend: invites (`POST /invites`, `GET /invites/{code}`, `POST /invites/{code}/redeem`), PIN reset (`POST /users/{id}/pin-resets`, `POST /auth/pin-resets/{code}/redeem`) and `hub reset-pin` from the server itself; `token_version` on every token with `POST /auth/refresh` to re-issue one; `POST /users/me/pin`; removal and leaving deactivate rather than delete, so what someone shared keeps their name, and agents know they have gone. Refusals carry a `code`, and 401 now means only that a token isn't accepted — a wrong PIN while signed in answers 403. One hub-wide guard covers guessing invite and reset codes. Client: a 401 forgets the token and returns to "Who's here?"; the app renews its token when it opens; the members screens are built as drawn, with the Profile shell holding only its backed rows until slice 6 fills it in. Codes carry `expires_in_seconds`, because a phone's clock need not agree with the hub's. Still to do by hand: the two-phone run in §8 of the slice plan, and an iPhone pass.
