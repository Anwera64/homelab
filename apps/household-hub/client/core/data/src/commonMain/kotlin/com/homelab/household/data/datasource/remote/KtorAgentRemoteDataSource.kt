...
class KtorAgentRemoteDataSource(
    ...
    override suspend fun listAgents(): List<AgentReadDto> =
        reachingHub {
            client.get("$baseUrl/api/v1/agents").ensureJsonSuccess().body()
        }

    override suspend fun getAgent(agentId: String): AgentReadDto =
        reachingHub {
            client.get("$baseUrl/api/v1/agents/$agentId").ensureJsonSuccess().body()
        }

    override suspend fun createAgent(agent: AgentCreateDto): AgentReadDto =
        reachingHub {
            client
    ...
    override suspend fun deleteAgent(agentId: String): Unit =
        reachingHub {
            client.delete("$baseUrl/api/v1/agents/$agentId").ensureJsonSuccess()
        }

    override suspend fun restoreAgent(agentId: String): AgentReadDto =
        reachingHub {
