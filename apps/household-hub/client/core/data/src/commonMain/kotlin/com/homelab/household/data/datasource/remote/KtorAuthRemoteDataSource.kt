...
class KtorAuthRemoteDataSource(
    ...
    override suspend fun onboard(
        ...
        reachingHub {
            val response =
                client.post("$baseUrl/api/v1/auth/register-initial") {
                    contentType(ContentType.Application.Json)
    ...
    override suspend fun lookUpInvite(code: String): InvitePreviewReadDto =
        reachingHub {
    ...
    override suspend fun joinHousehold(
        ...
        reachingHub {
            val response =
                client.post("$baseUrl/api/v1/invites/$code/redeem") {
                    contentType(ContentType.Application.Json)
    ...
    override suspend fun redeemPinReset(
        ...
        reachingHub {
            val response =
                client.post("$baseUrl/api/v1/auth/pin-resets/$code/redeem") {
                    contentType(ContentType.Application.Json)
    ...
    override suspend fun listMembers(): List<MemberProfileDto> =
        reachingHub {
            client
                .get("$baseUrl/api/v1/auth/members")
                .ensureJsonSuccess()
                .body<List<MemberProfileDto>>()
        }

    override suspend fun checkStatus(): AuthStatusDto =
        reachingHub {
            client.get("$baseUrl/api/v1/auth/status").ensureJsonSuccess().body()
        }

    override suspend fun fetchCurrentUser(): UserReadDto =
        reachingHub {
            client.get("$baseUrl/api/v1/auth/me").ensureJsonSuccess().body()
        }

    override suspend fun renew(accessToken: String): TokenResponseDto =
        reachingHub {
