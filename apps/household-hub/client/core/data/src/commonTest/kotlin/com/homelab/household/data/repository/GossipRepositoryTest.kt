...
class GossipRepositoryTest {
    ...
    @Test
    fun `GIVEN the hub reports a milestone WHEN the shared feed is asked for THEN it is mapped to a household milestone`() =
        runTest {
            // GIVEN
    ...
    @Test
    fun `GIVEN the hub cannot be reached WHEN the household's milestones are asked for THEN the failure reaches the caller`() =
        runTest {
            // GIVEN
            val remote = mock<GossipRemoteDataSource>()
    ...
    @Test
    fun `GIVEN a member auditing their own trail WHEN it is asked for THEN it is mapped to household milestones`() =
        runTest {
            // GIVEN
            val remote = mock<GossipRemoteDataSource>()
    ...
    @Test
    fun `GIVEN a milestone a member wants forgotten WHEN it is revoked THEN the data source is asked to remove it`() =
        runTest {
            // GIVEN
            val remote = mock<GossipRemoteDataSource>()
