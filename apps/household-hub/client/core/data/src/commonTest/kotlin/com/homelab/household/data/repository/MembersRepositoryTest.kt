...
class MembersRepositoryTest {
    ...
    @Test
    fun `GIVEN the hub lists one household member WHEN they are asked for THEN the profile is mapped to a user`() = runTest {
        // GIVEN
    ...
    @Test
    fun `GIVEN the hub cannot be reached WHEN household members are asked for THEN the failure reaches the caller`() = runTest {
        // GIVEN
        val remote = mock<MembersRemoteDataSource>()
    ...
    @Test
    fun `GIVEN the hub issues an invite code WHEN someone is invited THEN it is mapped to an invite`() = runTest {
        // GIVEN
        val remote = mock<MembersRemoteDataSource>()
        everySuspend { remote.createInvite("Liam", false) } returns InviteReadDto(
            code = "482913",
            invited_name = "Liam",
            is_admin = false,
    ...
    @Test
    fun `GIVEN a name the household already has WHEN someone is invited THEN the refusal reaches the caller`() = runTest {
        // GIVEN
        val remote = mock<MembersRemoteDataSource>()
    ...
    @Test
    fun `GIVEN the hub issues a reset code WHEN a forgotten PIN is approved THEN it is mapped to a reset code`() = runTest {
        // GIVEN
        val remote = mock<MembersRemoteDataSource>()
    ...
    @Test
    fun `GIVEN the approver's own PIN is wrong WHEN a reset is approved THEN the refusal reaches the caller`() = runTest {
        // GIVEN
        val remote = mock<MembersRemoteDataSource>()
    ...
    @Test
    fun `GIVEN a PIN change the hub accepts WHEN it is made THEN the fresh token replaces the kept one`() = runTest {
        // GIVEN
        val remote = mock<MembersRemoteDataSource>()
    ...
    @Test
    fun `GIVEN the wrong current PIN WHEN a change is attempted THEN the old token is left alone`() = runTest {
        // GIVEN
        val remote = mock<MembersRemoteDataSource>()
    ...
    @Test
    fun `GIVEN an admin removing someone WHEN the hub agrees THEN the hub is asked and nothing is kept`() = runTest {
        // GIVEN
        val remote = mock<MembersRemoteDataSource>()
    ...
    @Test
    fun `GIVEN the only admin WHEN someone tries to remove them THEN the refusal reaches the caller`() = runTest {
        // GIVEN
        val remote = mock<MembersRemoteDataSource>()
    ...
    @Test
    fun `GIVEN a member leaving WHEN they confirm with their PIN THEN the hub is asked`() = runTest {
        // GIVEN
        val remote = mock<MembersRemoteDataSource>()
    ...
    @Test
    fun `GIVEN the only admin WHEN they try to leave THEN the refusal reaches the caller`() = runTest {
        // GIVEN
        val remote = mock<MembersRemoteDataSource>()
