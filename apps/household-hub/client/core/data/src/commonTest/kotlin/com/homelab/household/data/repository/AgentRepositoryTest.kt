...
class AgentRepositoryTest {
    ...
    private val llamaDto = AgentReadDto(
        id = "a-1",
        slug = "llama",
        name = "Llama",
    ...
    private val llama = AgentPersonality(
        id = "a-1",
        slug = "llama",
        name = "Llama",
        description = "General helper",
    ...
    @Test
    fun `GIVEN the hub lists one agent WHEN its personalities are asked for THEN it is mapped to an agent personality`() = runTest {
        // GIVEN
    ...
    @Test
    fun `GIVEN the hub cannot be reached WHEN the household's agents are asked for THEN the failure reaches the caller`() = runTest {
        // GIVEN
        val remote = mock<AgentRemoteDataSource>()
    ...
    @Test
    fun `GIVEN one agent's id WHEN it is asked for by itself THEN it is mapped to an agent personality`() = runTest {
        // GIVEN
        val remote = mock<AgentRemoteDataSource>()
    ...
    @Test
    fun `GIVEN a new agent's personality WHEN it is created THEN it is sent as a create request and the stored agent comes back mapped`() = runTest {
        // GIVEN
        val remote = mock<AgentRemoteDataSource>()
        val expectedCreate = AgentCreateDto(
            slug = "llama",
            name = "Llama",
            description = "General helper",
            avatar = "🦙",
    ...
    @Test
    fun `GIVEN changes to an agent's personality WHEN it is updated THEN they are sent to that agent's own id and the result comes back mapped`() = runTest {
        // GIVEN
        val remote = mock<AgentRemoteDataSource>()
        val expectedUpdate = AgentUpdateDto(
            name = "Llama",
            description = "General helper",
            avatar = "🦙",
            system_prompt = "You are helpful.",
    ...
    @Test
    fun `GIVEN an agent an admin no longer wants WHEN it is deleted THEN the data source is asked to remove it`() = runTest {
        // GIVEN
        val remote = mock<AgentRemoteDataSource>()
    ...
    @Test
    fun `GIVEN a deleted agent WHEN it is restored THEN it is mapped to an agent personality`() = runTest {
        // GIVEN
        val remote = mock<AgentRemoteDataSource>()
