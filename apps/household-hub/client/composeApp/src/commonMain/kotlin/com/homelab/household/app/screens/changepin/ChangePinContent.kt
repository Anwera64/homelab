            ...
            BentoCard(modifier = Modifier.fillMaxWidth()) {
                Row(
                    ...
                    Column(verticalArrangement = Arrangement.spacedBy(HearthTheme.spacing.xxs)) {
                        Text(stringResource(Res.string.change_pin_warning), style = type.bodyStrong, color = colors.textPrimary)
                        Text(
...
@Composable
private fun currentErrorText(error: CurrentPinError): String = when (error) {
...
@Composable
private fun failure(status: ChangePinStatus): String? = when (status) {
