...
final class HearthLaunchTileTests: XCTestCase {
    ...
    /// Rasterizes `image` into an `canvasSize` × `canvasSize` RGBA8 bitmap at scale 1 and reads
    /// back the pixel at `point` (in the same unit space as `canvasSize`, i.e. the image's own
    /// drawing coordinates — not points or device pixels).
    ///
    /// The bitmap uses `.premultipliedLast` in `CGColorSpaceCreateDeviceRGB()`, which Quartz
    /// guarantees stores components in `R, G, B, A` byte order with no separate byte-order flag —
    /// so the read below needs no per-platform byte-order handling. `CGContext`'s native space has
    /// origin bottom-left with y increasing upward (PDF-style), while `UIImage.draw(in:)` — like
    /// all UIKit drawing — assumes origin top-left with y increasing downward; the translate +
    /// negative-y-scale before `UIGraphicsPushContext` converts one into the other so that, after
    /// drawing, offset 0 in the buffer is the *top-left* pixel and `point.y` grows downward, matching
    /// how `point` is written at each call site (and how `splash_tile.xml`'s own coordinates read).
    /// Letting `CGContext` allocate its own backing store (`data: nil`) avoids holding an
    /// escaping pointer into a Swift-managed buffer across the draw calls.
    private func rasterizedPixel(
        of image: UIImage,
        at point: CGPoint,
        canvasSize: CGFloat,
        file: StaticString = #filePath, line: UInt = #line
    ...
    /// Compares a sampled pixel against an 8-bit-per-channel hex triple with `tolerance` (default
    /// 8, i.e. ~3%). Unlike `HearthColorAssetTests.assertColor`'s half-quantisation-step tolerance
    /// — appropriate for a flat `UIColor` read straight out of `getRed:green:blue:alpha:` — a
    /// rasterized *vector* asset also carries antialiasing and colour-management slop from the
    /// draw and the DeviceRGB conversion, so a tighter tolerance would flake on correct assets.
